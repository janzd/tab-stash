...
- Only a loaded HTTP(S) tab in a focused normal window is eligible. Skip incognito, discarded/frozen, audible, split-view, pending navigation, and restricted/local pages.
...
A successful test does not establish zero performance impact on every website. Elapsed capture time is not CPU time. The prototype does not detect typing, muted video, screen sharing, or same-document visual changes. Inactive-tab updates do not reset the foreground settling timer. Full-screen Chrome windows are eligible; collection never leaves full screen. Settings shows the most recent collection outcome or skip/error reason. Already visited pages can have stale images; unvisited tabs have none. No timer refreshes a page that remains open without relevant events.
